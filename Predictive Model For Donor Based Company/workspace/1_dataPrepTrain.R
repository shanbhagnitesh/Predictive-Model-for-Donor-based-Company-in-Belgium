...
campaign_train <- read_delim("C:/Users/Nitesh_Shanbhag/Downloads/Machine_Learning/Final Project/data/campaign20130411.csv", 
...
# Take only data present till 2013, because the training data is till 2013-04-11
gifts$year <- year(gifts$date)
gifts <- subset(gifts, date < as.Date("2013-04-11"))
...
gifts <- gifts %>%
  group_by(donorID) %>%
  mutate(R_Years_Since_Last_Donation = 2013 - max(year),
         R_Last_Donation_Amount = sum(amount_gifts[date == max(date)]),
         R_Years_Since_First_Donation = 2013 - min(year),
         Freq_Donations = n(),
         Freq_Donations_3Years = max(ifelse(year %in% c(2010,2011,2012,2013),1,0)),
         Freq_Donations_5Years = max(ifelse(year %in% c(2008,2009,2010,2011,2012,2013),1,0)),
         Freq_Camps_Attended = n_distinct(campID),
         Freq_Camps_3Years = n_distinct(campID[year %in% c(2010,2011,2012,2013)]),
         Freq_Camps_5Years = n_distinct(campID[year %in% c(2008,2009,2010,2011,2012,2013)]),
         ...
         M_Total_Last_3Years = sum(amount_gifts[year %in% c(2010,2011,2012,2013)]),
         M_Total_Last_5Years = sum(amount_gifts[year %in% c(2008,2009,2010,2011,2012,2013)]))
